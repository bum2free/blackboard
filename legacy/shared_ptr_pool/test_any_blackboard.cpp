...
int main(void)
{
    ...
    std::vector<MyData1*> raw_ptrs_1;
    std::vector<MyData2*> raw_ptrs_2;
    for (size_t i = 0; i < NUM_OF_CONSUMERS + 2; ++i) {
        raw_ptrs_1.push_back(new MyData1());
        raw_ptrs_2.push_back(new MyData2());
    }

    // Register topics
    g_blackboard.register_topic<MyData1>("data1", raw_ptrs_1);
    g_blackboard.register_topic<MyData2>("data2", raw_ptrs_2);
